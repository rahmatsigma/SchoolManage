                ...
                <form action="{{ route('jadwal.update', $jadwal->id) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="jam_mulai" name="jam_mulai" type="time" class="mt-1 block w-full bg-white/10 text-white border-white/20" :value="old('jam_mulai', $jadwal->jam_mulai)" required />
                        </div>
                        <div>
                            <x-input-label for="jam_selesai" :value="__('Jam Selesai')" class="text-gray-300" />
                            <x-text-input id="jam_selesai" name="jam_selesai" type="time" class="mt-1 block w-full bg-white/10 text-white border-white/20" :value="old('jam_selesai', $jadwal->jam_selesai)" required />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="mata_pelajaran_id" id="mata_pelajaran_id" class="block mt-1 w-full bg-white/10 border border-white/20 text-white rounded-lg focus:border-blue-500 focus:ring-blue-500 [&>option]:text-gray-900">
                            ...
                            @foreach ($mataPelajaran as $mp)
                                <option value="{{ $mp->id }}" {{ $jadwal->mata_pelajaran_id == $mp->id ? 'selected' : '' }}>{{ $mp->nama_mapel }}</option>
                            @endforeach
                        </select>
                    </div>
